...
export default class ContactContentType extends Component {
    constructor(props) {
        super(props);
        ...
        // 不同頁面要不同路徑
        axios.post("/admin/contact_content_type_data").then(
            response => {
    ...
    destroyBtnFunction = (id, name) => {
        if (confirm(`是否要刪除 : ${name}`)) {
            // 不同頁面要不同路徑
            const path = `/admin/contact_content_type/${id}`
            ...
            isCreate
                ...
                isUpdate
                    ...
                    <div className="container">
                        <h1 className="display-3 text-center">連絡內容類型</h1>
                            ...
                            <tbody>
                                {tableData !== []
                                    ...
                                    tableData.map(data => {
                                        return (
                                            <tr key={data.id}>
                                                <td>{data.contact_type.name}</td>
                                                <td>{data.name}</td>
                                                <td>{data.contacts.length}</td>
